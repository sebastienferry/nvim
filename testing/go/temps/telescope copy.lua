return {
    ...
    branch = "0.1.x",
    config = function()
        localTelescopetelescope = require("telescope")
        ...
        map("n", "<leader>fh", builtin.help_tags, opts) -- Lists help tags
        map("n", "<leader>fg", builtin.live_grep, opts) -- Live grep
        map("n", "<leader>fr", builtin.lsp_references, opts) -- Lists LSP references
        map("n", "<leader>fd", builtin.lsp_definitions, opts) -- Lists LSP definitions
        map("n", "<leader>fi", builtin.lsp_implementations, opts) -- Lists LSP implementations
        map("n", "<leader>ft", builtin.lsp_type_definitions, opts) -- Lists LSP type definitions
        map("n", "<leader>fc", builtin.lsp_code_actions, opts) -- Lists LSP code actions
        map("n", "<leader>fm", builtin.lsp_document_symbols, opts) -- Lists LSP document symbols
        map("n", "<leader>fw", builtin.lsp_workspace_symbols, opts) -- Lists LSP workspace symbols
        map("n", "<leader>fq", builtin.quickfix, opts) -- Lists quickfix list
        map("n", "<leader>fl", builtin.loclist, opts) -- Lists location list
        map("n", "<leader>fo", builtin.oldfiles, opts) -- Lists oldfiles
    end,
